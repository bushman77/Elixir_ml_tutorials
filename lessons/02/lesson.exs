...
L2.info("vector", vector) |> L2.sample("vector", vector)
L2.info("matrix = iota({2,3})", matrix) |> L2.sample("matrix", matrix)
...
L2.info("ints (no type specified)", ints) |> L2.sample("ints", ints)
L2.info("floats (explicit float32)", floats) |> L2.sample("floats", floats)
...
L2.info("x (shape {4,3})", x) |> L2.sample("x", x, 4, 3)
...
L2.info("mean over axes: [1] (per-row)", row_means) |> L2.sample("row_means", row_means)

col_means_keep = Nx.mean(x, axes: [0], keep_axes: true) # shape {1,3}
L2.info("mean axes:[0], keep_axes:true (broadcast-friendly)", col_means_keep) |> L2.sample("col_means_keep", col_means_keep, 1, 3)
...
L2.info("xb (shape {#{n},#{d}})", xb) |> L2.sample("xb", xb, 6, 3)
L2.info("shift (shape {3})", shift) |> L2.sample("shift", shift)

# This will broadcast shift {3} across rows of xb {n,3}
xb_shifted = Nx.add(xb, shift)
L2.info("xb_shifted = Nx.add(xb, shift)  (broadcast shift across rows)", xb_shifted) |> L2.sample("xb_shifted", xb_shifted, 6, 3)

# Show an example that *fails* (shape mismatch):
bad = Nx.iota({n}, type: {:f, 32}) # shape {n}
L2.info("bad (shape {#{n}})", bad) |> L2.sample("bad", bad, 6)
...
L2.info("big (shape {10,5})", big) |> L2.sample("big", big, 6, 5)

first3_rows = Nx.slice(big, [0, 0], [3, 5])
L2.info("first3_rows = slice(big, [0,0], [3,5])", first3_rows) |> L2.sample("first3_rows", first3_rows, 3, 5)

col2 = Nx.slice(big, [0, 2], [10, 1])    # shape {10, 1}
col2_vec = Nx.squeeze(col2, axes: [1])   # shape {10}

L2.info("col2 = slice(big, [0,2], [10,1])", col2) |> L2.sample("col2", col2, 6, 1)
L2.info("col2_vec = squeeze(col2, axes:[1])", col2_vec) |> L2.sample("col2_vec", col2_vec, 10)

picked_rows = Nx.take(big, Nx.tensor([0, 3, 9]), axis: 0)
L2.info("picked_rows = take(big, [0,3,9], axis:0)", picked_rows) |> L2.sample("picked_rows", picked_rows, 3, 5)
...
L2.info("a (shape {2,6})", a) |> L2.sample("a", a, 2, 6)

a_reshaped = Nx.reshape(a, {3, 4})
L2.info("a_reshaped = reshape(a, {3,4})", a_reshaped) |> L2.sample("a_reshaped", a_reshaped, 3, 4)

a_t = Nx.transpose(a_reshaped) # {4,3}
L2.info("a_t = transpose(a_reshaped)", a_t) |> L2.sample("a_t", a_t, 4, 3)
...
L2.info("r", r) |> L2.sample("r", r, 4, 3)

sum_all = Nx.sum(r)
sum_cols = Nx.sum(r, axes: [0])
sum_rows = Nx.sum(r, axes: [1])

L2.info("sum_all = sum(r)  (shape {})", sum_all) |> L2.sample("sum_all", sum_all)
L2.info("sum_cols = sum(r, axes:[0])  (shape {3})", sum_cols) |> L2.sample("sum_cols", sum_cols)
L2.info("sum_rows = sum(r, axes:[1])  (shape {4})", sum_rows) |> L2.sample("sum_rows", sum_rows)
...
L2.info("x (dataset) shape {200,3}", x) |> L2.sample("x", x, 5, 3)
...
L2.info("mean (keep_axes) shape {1,3}", mean) |> L2.sample("mean", mean, 1, 3)
L2.info("std  (keep_axes) shape {1,3}", std) |> L2.sample("std", std, 1, 3)
L2.info("x_std = (x-mean)/std", x_std) |> L2.sample("x_std", x_std, 5, 3)

# Add a bias column of ones: {n, d} -> {n, d+1}
ones = Nx.broadcast(Nx.tensor(1.0, type: {:f, 32}), {n, 1})
x_bias = Nx.concatenate([x, ones], axis: 1)

L2.info("ones column shape {200,1}", ones) |> L2.sample("ones", ones, 5, 1)
L2.info("x_bias = concat([x, ones], axis:1) shape {200,4}", x_bias) |> L2.sample("x_bias", x_bias, 5, 4)
